//
//  LayerPropertiesVC.m
//  绘图
//
//  Created by qingyun on 16/1/9.
//  Copyright © 2016年 qingyun. All rights reserved.
//

#import "LayerPropertiesVC.h"
#import "maskLayer.h"

@interface LayerPropertiesVC ()
@property (nonatomic, strong) CALayer *layer;
@property (nonatomic, strong) CALayer *maskLayer;
@end

@implementation LayerPropertiesVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    //1.创建一个Layer
    CALayer *layer = [CALayer layer];
    //2.几何尺寸
    layer.bounds = CGRectMake(0, 0, 200, 200);
    layer.position = self.view.center;
    //3.外观
    layer.backgroundColor = [UIColor purpleColor].CGColor;
    
    _layer = layer;
    
    //4.添加layer
    [self.view.layer addSublayer:layer];
    
    _maskLayer = [CALayer layer];
    _maskLayer.contents = (__bridge id _Nullable)([UIImage imageNamed:@"jinmao"].CGImage);
    _maskLayer.bounds = CGRectMake(0, 0, 100, 100);
    _maskLayer.position = CGPointMake(100, 100);
}
- (IBAction)togoBorder:(id)sender {
    if (_layer.borderWidth > 0) {
        _layer.borderWidth = 0;
    }else{
        _layer.borderWidth = 5;
        _layer.borderColor = [UIColor blueColor].CGColor;
    }
}
- (IBAction)togoOpacity:(id)sender {
    if (_layer.opacity < 1) {
        _layer.opacity = 1;
    }else{
        _layer.opacity = 0.5;
    }
}
- (IBAction)togoShadow:(id)sender {
    if (_layer.shadowOpacity > 0) {
        _layer.shadowOpacity = 0;
    }else{
        _layer.shadowOpacity = 1;
        _layer.shadowRadius = 15;
    }
    
}
- (IBAction)togoSublayer:(id)sender {
    NSString *className = NSStringFromClass([maskLayer class]);
    NSLog(@"%@", className);
    if (_layer.sublayers == nil) {
        maskLayer *subLayer = [NSClassFromString(@"maskLayer") layer];
        subLayer.name = @"sublayer";
        subLayer.bounds = CGRectMake(0, 0, 100, 100);
        subLayer.position = CGPointMake(100, 100);
        
        [subLayer display];
        [_layer addSublayer:subLayer];
    }else{
        for (CALayer *layer in _layer.sublayers) {
            if ([layer isKindOfClass:[maskLayer class]]) {
                [layer removeFromSuperlayer];
            }
        }
    }
}

- (IBAction)togoMask:(id)sender {
    _layer.mask = _layer.mask == nil ? _maskLayer : nil;
}

- (void)didReceiveMemoryWarning {
    [super didReceiveMemoryWarning];
    // Dispose of any resources that can be recreated.
}

/*
#pragma mark - Navigation

// In a storyboard-based application, you will often want to do a little preparation before navigation
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    // Get the new view controller using [segue destinationViewController].
    // Pass the selected object to the new view controller.
}
*/

@end
